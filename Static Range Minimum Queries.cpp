...
ll get_min(ll *st, int ss, int se, int qs, int qe, int si){
    ...
    if(qs > se || qe < ss) return 999999999999999999999999;
    int mid = (ss + (se - ss) / 2);
    return(min(get_min(st, ss, mid, qs, qe, si * 2 + 1),get_min(st, mid + 1, se, qs, qe, si * 2 + 2)));
}
...
ll bld_uti(ll arr[], ll *st,int ss,int se,int si){
    ...
    int mid = (ss + (se - ss) / 2);
    st[si] =  min(bld_uti(arr, st, ss, mid, si*2+1), bld_uti(arr, st, mid + 1, se, si*2+2));
    return st[si];
}
...
int main()
{
    ...
    for(int i = 0 ; i < q; i++){
        int qt, a, b;
        cin >> a>> b;
        cout << get_min(st, 1, n, a, b,  1) << endl;
    }
}